...
export default function Footer(){
    ...
    const isActive = (path) => location.pathname.startsWith(path);
        ...
        useEffect(() => {
          ...
          try {
            ...
            setEmail(decoded.email); // ✅ สมมุติว่า backend ใส่ user_id มาใน token
    ...
    e.preventDefault();
    setIsSending(true);           // ✅ กำลังส่ง
    setSendStatus("");           // ล้างข้อความเก่า
    try {
        const res = await fetch(`${apiUrl}/api/send_contact`, {
        method: "POST",
        headers: {
        ...
        setSendStatus("เกิดข้อผิดพลาดในการส่ง ❌");
        }
    } catch (error) {
        setSendStatus("ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ได้ ❌");
        console.error(error);
    }
    setIsSending(false);         // ✅ เลิกแสดง "กำลังส่ง"
    };
    return(
                    ...
                    <h6 className="footer-title">ติดต่อหรือเสนอแนะ</h6>
                        <div className='flex flex-col items-end gap-3'>
                            <form
                                ...
                                className="flex flex-col items-end gap-2 w-[400px]"
                            >
                                <input
                                    type="text"
