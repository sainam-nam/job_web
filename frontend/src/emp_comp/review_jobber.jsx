...
import RatingReview from "../jobber_comp/RatingReview";
import Navbar from "./navbar";

const apiUrl = import.meta.env.VITE_API_BASE_URL;

function Review_jobber() {
  ...
  const [jobber_info , setJobber_info] = useState([]); 
  const [date , setDate] = useState([]); 

  useEffect(() => {
    const jobberfetchData = async () => {
      try {
        // console.log("📍 jobber_id ที่ส่งไป:", jobber_id);
        const res = await fetch(`${apiUrl}/user_profile?jobber_id=${jobber_id}`);
        const result = await res.json();
    
        if(Array.isArray(result.data)){
            //console.log("sql", res)
            setJobber_info(result.data);
  ...
  useEffect(() => {
    const dateaccpted = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/date_accptedjob?jobber_id=${jobber_id}&post_id=${id}`);
        const result = await res.json();
...
useEffect(() => {
    fetch(`${apiUrl}/jobb_rating?jobber_id=${jobber_id}`)
      .then(res => res.json())
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${apiUrl}/api/emp_review`, {
        jobber_id: jobber_id,
        emp_id: userId,
    ...
    return (
      <div>
        ...
        <div className="relative w-full group">
          <img src="/review.png" className="w-full" />
        </div>
        ...
        <div className="flex flex-row bg-white text-[#8E80FF] items-center justify-center gap-2  px-7 pb-5 md:px-20 md:pb-7 lg:px-30 lg:pb-14 xl:px-50 xl:pb-10 py-10"> 
          ...
          <div className="avatar">
            <div className="w-24 sm:w-28 md:w-35 lg:w-60 rounded-4xl">
              {jobber_info[0]?.picture ? (
                            <img src={`/uploads/user_pic/${jobber_info[0]?.picture}`} />
                          ) : (
                ...
                <div className="flex gap-5 items-center">  
                  <div className="flex flex-col">      
                    <a className="text-md md:text-xl lg:text-3xl xl:text-5xl font-bold">
                        {jobber_info[0]?.fullname}
                    </a>
                    <a className="text-md md:text-xl  font-bold">
                        {jobber_info[0]?.fullname_eng}
                    </a>      
                  </div>  
                    <button 
                        onClick={() => goToProfile(jobber_id)}
                        className="btn btn-lg text-white bg-[#8E80FF] border border-[#8E80FF] rounded-xl p-1 md:p-2 lg:p-3"
                    >ดูโปร์ไฟล์</button>      
                    </div>
              {stars !== 0 && (
              ...
              <table>
                <tbody>
                  <tr className="lg:h-10 xl:h-12">
                    ...
                    <td><a className="text-[10px] md:text-sm lg:text-xl text-[#8E80FF] font-bold">เพศ</a></td>
                    <td>
                      <a className="text-[10px] md:text-sm lg:text-xl text-[#8E80FF]">
                      {jobber_info[0]?.gender === "M"
                      ? "ชาย"
                    : jobber_info[0]?.gender === "F"
                      ? "หญิง"
                    : "ไม่ระบุเพศ"}
                      </a>
                    </td>
                  ...
                  <tr className="lg:h-10 xl:h-12">
                    <td><FaCircle color="#8E80FF" size={12} /></td>
                    <td><a className="text-[10px] md:text-sm lg:text-xl text-[#8E80FF] font-bold">ที่อยู่ผู้ทำงาน</a></td>
                    <td><a className="text-[10px] md:text-sm lg:text-xl text-[#8E80FF]">ต.{jobber_info[0]?.tb} อ.{jobber_info[0]?.ap} จ.{jobber_info[0]?.jw}</a></td>
                  </tr>
                </tbody>
              </table>
