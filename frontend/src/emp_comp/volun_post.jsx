...
function Emp_Volun_Post() {
  ...
  const [volunHs, setVolunHS] = useState([]);
  const [volunSs, setVolunSS] = useState([]);
  ...
  const handleUpdate = async () => {
    ...
    const payload = {
      ...
      volunteer_code: formData.volunteer_code,
      ...
      how_to_join: formData.how_to_join ?? "",
    ...
    try {
      const res = await axios.put(`${apiUrl}/api/volun_posting_update/${id}`, payload);
  ...
  useEffect(() => {
          fetchData();
          volunMatchFetchData(id);
          checkIfClose();
        }, [id , closed , selectedJobberId , message , status , empPercent]);

  async function checkIfClose() {
    try {
      const res = await fetch(`${apiUrl}/api/voluncheck-close?post_id=${id}`);
  ...
  async function handleCloseToggle(post_id) {
    try {
      if (closed) {
        // ลบออก
        await fetch(`${apiUrl}/api/volunupdate-status`, {
          method: "PATCH",
  ...
  function mapJobToFormData(volun, volun_HS = [], volun_SS = []) {
    // แยก min_age, max_age จาก field "age"
    // console.log("ข้อมูลในแมท",job);
    let min_age = "";
    let max_age = "";
    if (volun.age && volun.age.includes("-")) {
      const [min, max] = volun.age.split("-");
      min_age = min || "";
      max_age = max || "";
    }
    ...
    let email = "";
    if (volun.contact) {
      const parts = volun.contact.split(" / ");
      phone = parts[0]?.trim() || "";
      email = parts[1]?.trim() || "";
    }
    let start = "";
    let end = "";
    if (volun.time) {
      const parts = volun.time.split(" - ");
      start = parts[0]?.trim() || "";
      end = parts[1]?.trim() || "";
    }
//ยังไม่ค่อยสมนะ
    return {
      num_position: volun.num_position ?? "1",
      volunteer_code: volun.volunteer_code ?? "",
      activity_name: volun.activity_name ?? "",

      address: 0, // backend ไม่มีส่ง address
      location: volun.location ?? "",
      tambon_id: volun.tambon_id ?? "",
      ampher_id: volun.tambon_id ? volun.tambon_id.toString().substring(0, 4) : "",
      jangwat_id: volun.tambon_id ? volun.tambon_id.toString().substring(0, 2) : "",
      latitude: volun.latitude ?? "",
      longitude: volun.longitude ?? "",

      details: volun.details ?? "",
      min_age,
      max_age,
      gender: volun.gender ?? "A",

      experience: volun.experience ?? "",
      notes: volun.notes ?? "",
      how_to_join: volun.how_to_join ?? "",
      prepare: volun.prepare ?? "",

      ownbenefit: 0,
      date_start: volun.date_start ?? "",
      date_end: volun.date_end ?? "",
      owncontact: 0,

      time: start,
      end_time: end,

      selectedHs: volun_HS.map((h) => h.hardskill_id?.toString() || ""),
      selectedSs: volun_SS.map((s) => s.softskill_id?.toString() || ""),
      phone,
      email,
    };
  }
...
function decodeBitString(bitString, order) {
  ...
  const fetchData = async () => {
    try {
      ...
      if(Array.isArray(result.volunpost)){
          ...
          const volun = result.volunpost[0]; // ✅ เอา record แรกมาใส่ในฟอร์ม
          setRemain(volun.num_position - volun.accepted_count);
          // console.log("formDataก่อนแมท",job);


          const mapped = mapJobToFormData(volun, result.volun_HS, result.volun_SS);
          setFormData(mapped);
  ...
  const goToProfile = (post_id , jobber_id , matchPer , status , message , inter_volun_id) => {
    navigate(`/volun_match_de?pi=${post_id}&ji=${jobber_id}&matchper=${matchPer}&status=${status}&message=${message}&inter_id=${inter_volun_id}`);
    window.scrollTo(0,0);
  };

  const volunMatchFetchData = async (id) => {
    try {
      const apiUrl = import.meta.env.VITE_API_BASE_URL;

      const res = await fetch(`${apiUrl}/emp_volun_match?post_id=${id}&page=${page}&limit=${limit}&percent=${empPercent}`);
      const result = await res.json();
  ...
  const handleFileChange = (e) => {
    ...
    const handleUpload = async () => {
        ...
        formData.append("volun_pic", selectedImage);
        formData.append("post_id", id); // ถ้าต้องการส่ง user ID ไปด้วย

        try {
        const res = await axios.post(`${apiUrl}/api/volunpost_up_picture`, formData, {
            headers: {
  ...
  const [avLimit, setAjLimit] = useState(3);
  const [applyVolun, setApplyVolun] = useState([]);
  const [avTotal, setAvTotal] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`${apiUrl}/api/volun_history?post_id=${id}&page=${avPage}&limit=${avLimit}`);
        const result = await res.json();
        //console.log("ข้อมูลลลลลลลลลลลลลลล",result); 
        
        
        if(Array.isArray(result.apply_volun)){
          //console.log("sql", res)
          setApplyVolun(result.apply_volun);
          //console.log(result.data);
        } else {
          console.error("Data format error:", result);
          setApplyVolun([]);
        }
        setAvTotal(result.avtotalCount);
        setAvTotalPages(result.avtotalPages || 1);
    ...
    fetchData();
  }, [ id , avPage , avLimit , message , status , selectedJobberId]);
      ...
      <div>
        {userData && <Navbar user={userData} />}
          <div className="relative w-full group">
            <img src="/volunpost.png" className="w-full" />
              <button
                onClick={() => navigate(-1)}
          ...
          <div id="section1" className="flex flex-col justify-center bg-white px-5 py-3 md:px-15 md:py-6 lg:px-25 lg:py-12 xl:px-35 xl:pt-14">
            <div className="flex flex-col items-center w-full">
              <div  className="flex justify-center">
                <p className="text-[#7B6ADA] text-xs md:text-xl lg:text-4xl font-bold">{data[0]?.activity_name}</p>
                    <button 
                      onClick={() => {
                        setShowWork(prev => {
          ...
          {showWork === 1 && (
            <div  className="bg-white px-5 py-3 md:px-15 md:py-6 lg:px-25 lg:py-12 xl:px-35 xl:pb-14">
              <div className="flex flex-col justify-center w-full mb-2 p-4 md:mb-4 md:p-8 lg:mb-8 bg-gray-200 text-[#7B6ADA] rounded-3xl lg:rounded-4xl" style={{ boxShadow: '0 0 10px rgba(0,0,0,0.2)' }}>
                ...
                <div className="flex items-center justify-center gap-5">
                  ...
                  >
                    {data[0]?.volun_pic ? (
                      <img
                        src={`/uploads/emp_pic/${data[0]?.volun_pic}`}
                  ...
                  <table>
                    <tbody>
                      <tr className="lg:h-10 xl:h-12">
                        ...
                        <td className="w-20 md:w-40 lg:w-50 xl:w-60"><a className="text-[10px] md:text-sm lg:text-xl text-[#7B6ADA]">วันที่ทำกิจกรรม</a></td>
                        <td className="w-40 md:w-80 lg:w-90 xl:w-100"><a className="text-[10px] md:text-sm lg:text-xl text-[#7B6ADA]">{data[0]?.date_start && formatDateToThaiShort(data[0].date_start)}{data[0]?.date_end && `-`+formatDateToThaiShort(data[0].date_end)}</a></td>
                      </tr>
                      <tr className="lg:h-10 xl:h-12">
                        <td className="w-5 md:w-10 lg:w-10 xl:w-15"><FaCircle color="#7B6ADA" size={12} /></td>
                        <td className="w-20 md:w-40 lg:w-50 xl:w-60"><a className="text-[10px] md:text-sm lg:text-xl text-[#7B6ADA]">เวลาที่ทำกิจกรรม</a></td>
                        <td className="w-40 md:w-80 lg:w-90 xl:w-100"><a className="text-[10px] md:text-sm lg:text-xl text-[#7B6ADA]">{data[0]?.time && data[0].time}</a></td>
                      </tr>
                      <tr className="lg:h-10 xl:h-12">
                        <td><FaCircle color="#7B6ADA" size={12} /></td>
                        <td><a className="text-[10px] md:text-sm lg:text-xl text-[#7B6ADA]">สถานที่ทำกิจกรรม</a></td>
              ...
              <div className="py-2 md:py-4 lg:py-6">
                  ...
                  {data[0]?.prepare && (
                    <>
                      <p className="text-sm font-bold lg:text-2xl xl:text-3xl text-[#7B6ADA] ml-2 mb-1 md:ml-4 md:mb-2">สิ่งที่ต้องเตรียม</p>
                      <div className="text-[10px] md:text-sm lg:text-xl xl:text-2xl text-[#7B6ADA] ml-4 md:ml-10">
                        <div
                          className="prose prose-sm md:prose lg:prose-lg"
                                                                  ...
                                                                  <div className="flex items-start w-auto space-x-6">
                                                                    ...
                                                                    <div className="flex flex-col items-start w-3/4 text-sm text-gray-700">
                                                                      <div className="font-semibold text-[#8E80FF] text-2xl">
                                                                        ...
                                                                        {/* ใส่ status ให้ว่า มีการสมัครเข้ามาแล้ว  ไปแก้ฝั่งjobber ด้วย ตอนกดสมัคร แล้วตอนค้นหาก็ไม่ต้องเอากิจกรรมนี้ออกมาหรือเปล่า หรือออกมาแต่ตอนที่กดดูรายละเอียดกว่ามีการแม็ทก็ขึ้นมาด้วย */}
                                                                      </div>
                                      ...
                                      <>
                                        ...
                                        <div className="flex flex-col justify-center items-center mx-3 py-10 sm:p-4 md:px-8 lg:pt-12 lg:px-12 xl:px-40 text-center">
                                          ...
                                          <p className="text-[#8E80FF] text-base md:text-lg lg:text-xl font-semibold">
                                            ยังไม่มีคนสมัครกิจกรรมที่เหมาะกับกิจกรรมของคุณ
                                          </p>
                                          <p className="text-[#8E80FF] text-sm md:text-md mt-1 mb-5">
                                    ...
                                    {applyVolun.length > 0 ? (
                                                    ...
                                                    {/* /end flex  job/ */}
                                                            ...
                                                            <center>
                                                              <div className="join items-center gap-2 mb-3">
                                                                ...
                                                                {/* ปุ่มเลขเพจ */}
                                                                {Array.from({ length: avTotalPages }, (_, i) => i + 1).map((pageNum) => (
                                                                  <button
                                                                    key={pageNum}
                                                                    onClick={() => setAvPage(pageNum)}
                                                                    className={`btn btn-xs rounded-3xl join-item transition-colors
                                                                      ${
                                                                        avPage === pageNum
                                                                          ? "bg-[#6C5CE7] border-[#6C5CE7] text-white hover:bg-[#5945c7]"
                                      ...
                                      <>
                                        ...
                                        <div className="flex flex-col justify-center items-center mx-3 py-10 sm:p-4 md:px-8 lg:pt-12 lg:px-12 xl:px-40 text-center">
                                          ...
                                          <p className="text-[#8E80FF] text-base md:text-lg lg:text-xl font-semibold">
                                            ยังไม่มีคนสมัครกิจกรรมที่สมัครกิจกรรมของคุณ
                                          </p>
                                    ...
                                    {favorite.length > 0 ? (
                                      ...
                                      <>
                                        ...
                                        <div className="flex flex-col justify-center items-center mx-3 py-10 sm:p-4 md:px-8 lg:pt-12 lg:px-12 xl:px-40 text-center">
                                          ...
                                          <p className="text-[#8E80FF] text-base md:text-lg lg:text-xl font-semibold">
                                            ยังไม่มีคนสมัครกิจกรรมที่สนใจกิจกรรมของคุณ
                                          </p>
                                          {/* <p className="text-gray-100 text-sm md:text-md mt-1">
                                            อย่าเพิ่งกังวลนะคะ เราจะคอยอัปเดตให้ทันทีเมื่อมีผู้สมัครที่ตรงตามคุณสมบัติ 💜
                                          </p> */}
        ...
        <dialog id="close_modal" className="modal">
          <div className="modal-box bg-white rounded-2xl shadow-xl">
            ...
            <div className="modal-action flex justify-center gap-4">
              <button
                className="btn btn-sm md:btn-md bg-red-400 hover:bg-red-700 border border-red-600 text-white px-5 py-2 rounded-lg shadow-md transition-all"
        ...
        <dialog id="up_pic_modal" className="modal">
          <div className="modal-box bg-white rounded-3xl">
            ...
            <div className="modal-action flex justify-center">
              ...
              {/* Modal สำหรับแก้ไขข้อมูลกิจกรรม */}
                                      ...
                                      <form onSubmit={(e) => {handleGoGo(e);}} className='mt-3 '>
                                                  ...
                                                  <>
                                                    <div className="flex flex-col gap-1 text-sm w-2/3">
                                                      ...
                                                      <div className="flex justify-center mt-2 font-bold">
                                                            ประเภทกิจกรรม : {data[0]?.voluntype_name}
                                                        </div>
                                                    
                                                    <div className="flex justify-between  mt-2">
                                                                                                  <TooltipField 
                                                                                                    label="ชื่อกิจกรรม" 
                                                                                                    tooltip="ชื่อกิจกรรมควรบ่งบอกถึงกิจกรรมจิตอาสาที่ผู้ลงทะเบียนจะเข้าใจ"
                                                                                                  ></TooltipField>
                                                                                            </div>
                                              ...
                                              <input 
                                                  ...
                                                  <div className="flex items-end gap-2 mt-2">
                                                                                                          <a className="text-[#8E80FF] font-bold">รายละเอียดกิจกรรมจิตอาสา</a>
                                                                                                          {/* <a className="text-[#A3A3A3] text-xs">รายละเอียดกิจกรรม</a> */}
                                                                                                      </div>
                                                                                      ...
                                                                                      <div className="flex gap-2 mt-2">
                                                                                                          <a className="text-[#8E80FF] font-bold">วิธีการเข้าร่วม</a>
                                                                                                          
                                                                                                      </div>
                                                                                                                          
                                                                                                      <TiptapEditor
                                                                                                        value={formData.how_to_join}
                                                                                                        onEditorChange={(content) => handleEditorChange("how_to_join", content)}
                                                                                                      />
                                                  ...
                                                  <>
                                                    <div className="flex flex-col gap-1 text-sm w-2/3">
                                                      <div className="flex items-center gap-2 mt-2">
                                                            <a className="text-[#8E80FF] font-bold">สถานที่ทำกิจกรรม</a>
                                                            <label className="label pt-1">
                                                              ...
                                                              <div className="flex gap-2">
                                                                ...
                                                                <div className="flex gap-2">
                                                                  <div className="flex flex-col w-full">
                                                                      <div className="flex items-center justify-start text-[#8E80FF] font-bold">
                                                                        ...
                                                                        <TooltipField 
                                                                          label="" 
                                                                          tooltip={`ใช้กรณีอยากให้แสดงกิจกรรมบนแผนที่\nสามารถ copy พิกัดจาก Google Maps`}
                                                                        ></TooltipField>
                                                                      </div>
                                                                          <input 
                                                                              type="number" 
                                                                ...
                                                                <div className="flex gap-2">
                                                  <div className="flex flex-col w-full">
                                                      <div className="text-[#8E80FF] font-bold">เวลาที่จัดกิจกรรม <a className="text-xs">( ตั้งแต่ )</a></div>
                                                          <input 
                                                              type="time" 
                                             ...
                                             <div className="flex gap-2">
                                                  <div className="flex flex-col w-full">
                                                      <div className="text-[#8E80FF] font-bold">วันที่จัดกิจกรรม <a className="text-xs">( ตั้งแต่ )</a></div>
                                                          <input 
                                                              type="date" 
                                                              className="input w-full bg-[#8E80FF] text-white border-[#A3A3A3] rounded-box" 
                                                              name="date_start"
                                                  ...
                                                  <>
                                                    <div className="flex flex-col gap-1 text-sm w-2/3">
                                                      ...
                                                      <div className="flex items-end gap-2 mt-2">
                                                                    ...
                                                                    />
                                                      <div className="flex items-end gap-2 mt-2">
                                                                                                              <a className="text-[#8E80FF] font-bold">สิ่งที่ต้องเตรียม</a>
                                                                                                              {/* <a className="text-[#A3A3A3] text-xs">รายละเอียดกิจกรรม</a> */}
                                                                                                          </div>
                                                                                                          <TiptapEditor
                                                                                                            value={formData.prepare}
                                                                                                            onEditorChange={(content) => handleEditorChange("prepare", content)}
                                                                                                          /> 
                                                      <div className="flex justify-between  mt-2">
            ...
            <div className="modal-box bg-white">
                <center>
                    <p className="text-4xl  text-[#8E80FF]">ยืนยันการลงประกาศกิจกรรม</p>
