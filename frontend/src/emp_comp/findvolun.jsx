...
import VolunCard from "./volun_card";
...
function FindVolun() {
  ...
  const editor = useEditor({
    ...
    content: "<p>เริ่มพิมพ์เนื้อหากิจกรรมจิตอาสาที่นี่...</p>",
  });
  ...
  const [formData, setFormData] = useState(() => {
    const saved = localStorage.getItem("volunForm");
    return saved
      ? {
          selectedHs: [""],
          selectedSs: [""],
  ...
  const handleResetFilters = () => {
    ...
    localStorage.removeItem("volunForm"); // ถ้าอยากเคลียร์ localStorage ด้วย
  };
      ...
      const handleFileChange = (e) => {
        ...
        if (file) {
          setFormData((prev) => ({
            ...prev,
            volun_pic: file,   // เก็บไฟล์ใน state โดยตรง
          }));
        }
      };
  ...
  useEffect(() => {
    localStorage.setItem("volunForm", JSON.stringify(formData));
  }, [formData]);
...
//ข้อมูลยังไม่เข้า แต่ข้อมูล เปลี่ยนแท็บแล้วยังอยู่ดี ผ่าน แต่ถ้ารีเฟรชอาจจะหายไปบางข้อมูล
  ...
  const handleSubmit = async () => {
            const form = new FormData();
              ...
              try {
                form.append("num_position", formData.num_position ?? 1);
                form.append("volunteer_code", formData.volunteer_code ?? "");
                form.append("activity_name", formData.activity_name ?? "");

                form.append("address", formData.address ?? 0);
                form.append("location", formData.location ?? "");
                form.append("tambon_id", formData.tambon_id ?? "");
                form.append("ampher_id", formData.ampher_id ?? "");
                form.append("jangwat_id", formData.jangwat_id ?? "");
                ...
                form.append("gender", formData.gender ?? "A");

                form.append("experience", formData.experience ?? "");
                form.append("notes", formData.notes ?? "");
                form.append("how_to_join", formData.how_to_join ?? "");
                form.append("prepare", formData.prepare ?? "");

                form.append("time", formData.time ?? "");
                form.append("end_time", formData.end_time ?? "");
                form.append("date_start", formData.date_start ?? "");
                form.append("date_end", formData.date_end ?? "");
                ...
                if (formData.volun_pic instanceof File) {
                  form.append("volun_pic", formData.volun_pic);
                }
                ...
                const res = await axios.post(`${apiUrl}/api/volun_posting_add`, form);
...
useEffect(() => {
  ...
  const MJfetchData = async () => {
    ...
    if (status === "p") {
      endpoint = `${apiUrl}/my_volun?emp_id=${userId}&page=${MJpage}&limit=${MJlimit}&keyword=${search}&status=ON`;
    } else if (status === "f") {
      endpoint = `${apiUrl}/my_volun?emp_id=${userId}&page=${MJpage}&limit=${MJlimit}&keyword=${search}&status=Finish`;
    } else {
    ...
    useEffect(() => {
      ...
      useEffect(() => {
          ...
          const stepLabels = [
              ...
              "คุณสมบัติของผู้เข้ร่วมกิจกรรม",
              "ข้อมูลการติดต่อ"
          ];

          const stepCompletion = [
            // step 1
            // (formData.volun_pic instanceof File || !!formData.volun_pic) &&
            !!formData.volunteer_code &&
            (formData.num_position !== null && formData.num_position !== undefined && formData.num_position !== ""),
            ...
            // step 3
            formData.min_age !== "" && formData.max_age !== "" && !!formData.gender,
            ...
            <>
              <div className="flex flex-col items-center w-full gap-2 bg-white">
                              ...
                              <div className="flex flex-col justify-center items-center w-full px-5 pt-3 lg:pt-5 lg:px-0 lg:pr-4">
                                <h1 className="text-lg md:text-3xl font-bold mb-2 text-[#8E80FF] my-3">
                                  {status === "p" ? (
                                    <>กิจกรรมที่ประกาศไว้ [ {MJtotal ? (MJtotal):("0")} ]</>
                                  ) : (
                                    <>กิจกรรมที่เสร็จสิ้นไปแล้ว [ {MJtotal ? (MJtotal):("0")} ]</>
                                  ) }
                                </h1>
                                                      ...
                                                      <div className="mb-3">
                                                        {MJdata.length === 0 && status === 'f' ? (
                                                          ...
                                                          <div className="flex flex-col items-center">
                                                            {MJdata.length > 0 && (
                                                              <label className="input w-50 md:w-90 h-7 lg:h-9 bg-white my-5 rounded-lg border border-[#8E80FF]">
                                                                <button>
                                                                ...
                                                                <input 
                                                                  ...
                                                                  placeholder="ชื่อกิจกรรม หรือ ประเภทกิจกรรม" 
                                                                  value={search}
                                                                  onChange={(e) => {
              ...
              <div className="flex flex-col gap-3 mt-10">
                  ...
                  <div className="flex flex-col lg:flex-row justify-center px-50 w-full">
                    ...
                    <div className="text-[#8E80FF] text-xs">
                        <div className="text-[#8E80FF] text-xs leading-relaxed mt-2 px-1">
                          <p className="font-bold">คำแนะนำการลงประกาศกิจกรรมจิตอาสา:</p>
                          <ul className="list-disc list-inside space-y-1">
                            <li>
                              <span className="font-semibold">รายละเอียดกิจกรรม:</span> 
                              อธิบายลักษณะของกิจกรรม วัตถุประสงค์ และสิ่งที่จะทำอย่างชัดเจน
                            </li>
                            <li>
                              <span className="font-semibold">สถานที่และช่วงเวลาจัดกิจกรรม:</span> 
                              ระบุสถานที่จัดกิจกรรม วันที่ และเวลาเริ่ม-สิ้นสุดอย่างครบถ้วน
                            </li>
                            <li>
                              <span className="font-semibold">คุณสมบัติของผู้เข้าร่วม:</span> 
                              เช่น อายุ เพศ ความสามารถ หรือประสบการณ์ที่เหมาะสมกับกิจกรรม
                            </li>
                            <li>
                              <span className="font-semibold">การเตรียมตัวและสิ่งที่ต้องนำมา:</span> 
                              แจ้งผู้เข้าร่วมว่าควรเตรียมอุปกรณ์หรือสิ่งของใดบ้างก่อนเข้าร่วม
                            </li>
                            <li>
                              <span className="font-semibold">ข้อมูลการติดต่อ:</span> 
                              ใส่เบอร์โทร อีเมล หรือช่องทางที่สามารถสอบถามข้อมูลเพิ่มเติมได้
                            </li>
                          </ul>
                          <p className="font-semibold mt-2">
                            ควรกรอกข้อมูลกิจกรรมให้ครบถ้วนเพื่อให้ผู้สนใจเข้าใจและตัดสินใจเข้าร่วมได้ง่ายขึ้น
                          </p>
                        </div>
                  ...
                  <div id="section_post" className="flex flex-col w-full gap-2">
                    <h1 className="text-center text-lg md:text-4xl font-bold mb-2 text-[#8E80FF] my-3">ลงประกาศกิจกรรมจิตอาสา</h1>
                    ...
                    <div className='text-[#8E80FF] bg-white rounded-b-3xl rounded-tr-3xl p-5 w-full' style={{ boxShadow: '0 0 10px rgba(0,0,0,0.2)' }}>
                      ...
                      <div className="flex justify-center items-center gap-3">
                        <div className='text-center pl-4 font-bold text-xl'>
                          {currentStep === 1 && "รายละเอียดกิจกรรม"}
                          {currentStep === 2 && "สถานที่และเวลาทำกิจกรรม"}
                          {currentStep === 3 && "คุณสมบัติของผู้เข้าร่วมกิจกรรม"}
                                ...
                                {currentStep === 1 && (
                                  <>
                                    <div className="flex flex-col gap-1 text-sm w-2/3">
                                        <div className="flex flex-col gap-2 mt-2">
                                          {formData.job_pic instanceof File && (
                                            ...
                                            <div className="flex">
                                              ...
                                              <TooltipField 
                                                label="เพิ่มรูปภาพ" 
                                                tooltip={`ไฟล์รูปภาพกิจกรรม/ประกาศกิจกรรม\n(เช่น ภาพประกอบกิจกรรม)`}
                                              >
                                              <input
                                                name="job_pic"
                                                type="file"
                                        ...
                                        <div className="flex justify-between  mt-2">
                                              <TooltipField 
                                                label="ประเภทกิจกรรมจิตอาสา" 
                                                tooltip="เลือกประเภทกิจกรรมจิตอาสาที่คุณต้องการ"
                                              ></TooltipField>
                                        </div>
                                        ...
                                        <select
                                          ...
                                          onChange={(e) => setFormData({ ...formData, volunteer_code: e.target.value })}
                                      >
                                          <option value="">-- กรุณาเลือกประเภทกิจกรรมจิตอาสา --</option>
                                          {Array.isArray(voluntypeList) && voluntypeList.map((vt) => (
                                              <option key={vt.voluntype_id} value={vt.voluntype_id} className='text-[#8E80FF]'>
                                                  {vt.voluntype_name}
                                              </option>
                                              ))}
                                      </select>
                                    ...
                                    <div className="flex flex-col gap-1 text-sm w-2/3">
                                      <div className="flex items-center gap-2 mt-2">
                                            ...
                                            <label className="label pt-1">
                                                  ...
                                                  />
                                                  <a className='text-xs'>ใช้ที่อยู่ของผู้จัดกิจกรรมเอง</a>
                                              </label>
                                        </div>
                                                ...
                                                <div className="flex gap-2">
                                                  <div className="flex flex-col w-full">
                                                      <div className="flex items-center justify-start text-[#8E80FF] font-bold">
                                                        ...
                                                        <TooltipField 
                                                          label="" 
                                                          tooltip={`ใช้กรณีอยากให้แสดงสถานที่บนแผนที่\nสามารถ copy พิกัดจาก Google Maps`}
                                                        ></TooltipField>
                                                      </div>
                                                          <input 
                                                              type="number" 
                                          ...
                                          <div key={index} className="flex gap-2 items-center">
                                            ...
                                            {index === 0 ? (
                                              <button
                                                type="button"
                                                onClick={addHs}
                                                className="btn btn-sm bg-green-600 text-white btn-outline w-8 rounded-xl text-xl"
                                              >
                                                +
                                              </button>
                                            ) : (
                                              <button
                                                ...
                                                className="btn btn-sm bg-red-600 text-white btn-outline w-8 rounded-xl text-xl"
                                              >
                                                -
                                              </button>
                                            )}
              ...
              <div className="modal-box bg-white">
                ...
                <div className="modal-action flex justify-center">
                  <button className="btn bg-[#8E80FF] text-white" 
                    onClick={() => {navigate(`/Emp_Volun_Post?pi=${newPost_id}`);
                    window.scrollTo(0,0);}}>
