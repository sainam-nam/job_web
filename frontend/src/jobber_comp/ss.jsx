...
export default function UserSs() {
  ...
  const [addData, setAddData] = useState({ softskill_id: "", jobber_id: userId });
  const [delData, setDelData] = useState({ softskill_id: "", jobber_id: userId });
  ...
  if (userId) {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/user_softskill?page=${page}&limit=${limit}&keyword=${search}&jobber_id=${userId}`);
        ...
        const userSsIds = result.data.map((ss) => ss.softskill_id);

        // 👇 filter hsData ทันที (ลบทักษะที่ผู้ใช้มีออกจาก dropdown)
        setSsData((prev) => prev.filter((ss) => !userSsIds.includes(ss.softskill_id)));
    ...
    const ss_data = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/ss`);
        const result = await res.json();
  
        if(Array.isArray(result.data)){
          //console.log("sql", result)
          setSsData(result.data);
        } else {
          console.error("Data format error:", result);
          setSsData([]);
        }
    ...
    const handleInsert = async () => {
      ...
      try {
        ...
        const res = await fetch(`${apiUrl}/add_jobber_ss/`, {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ 
            jobber_id: userId, 
            softskill_id: addData.softskill_id
          }),
        });
  
        if (res.ok) {
          setSsData((prev) =>
            prev.filter((ss) => ss.softskill_id !== addData.softskill_id)
          );
    ...
    const handleDelete = async () => {
      try {
        ...
        const res = await fetch(`${apiUrl}/user_softskill/?jobber_id=${userId}&softskill_id=${delData.softskill_id}`, {
          method: "DELETE",
      ...
      <div className="flex flex-col rounded-3xl p-2"  style={{ boxShadow: '0 0 10px rgba(0,0,0,0.2)' }}>
        <div className="flex justify-between items-center mt-1 mb-1 md:mt-2 md:mb-2">
          <div className="flex gap-2">
            <h1 className="text-white text-[10px] font-bold md:text-lg">คุณมีทักษะด้านอารมณ์ทั้งหมด {total} รายการ</h1>
          ...
          <div className="flex items-center gap-2">
            <label className="input w-30 h-7 bg-white rounded-lg border border-[#8E80FF] ">
              <div className="tooltip tooltip-left" data-tip="ค้นหาจากทักษะด้านอารมณ์">
                <input 
                  type="search" 
        ...
        <center>
          ...
          {page > 1 && (
              <button disabled={page === 1} onClick={() => setPage(page - 1)}><img src="/up.png" className="-rotate-90 w-4 h-4 md:w-6 md:h-6 hover:shadow-lg hover:shadow-[#8E80FF] transition-shadow" /></button>
          )}
          {total > 0 ? (
          ...
          {page < totalPages && (
            <button disabled={page === totalPages} onClick={() => setPage(page + 1)}><img src="/down.png" className="-rotate-90 w-4 h-4 md:w-6 md:h-6 hover:shadow-lg hover:shadow-[#8E80FF] transition-shadow" /></button>
          )}
          </div>
        </center>
      </div>
            ...
            <dialog id="add_modal" className="modal">
              <div className="modal-box bg-white">
                ...
                <select 
                  ...
                  value={addData.softskill_id}
                  onChange={(e) => setAddData({...addData, softskill_id: e.target.value })}
                >
                  <option value="">--เลือกประเภทงานที่ตรงกับตำแหน่งงาน--</option>
                  {ssData.map((ss) => (
                    <option key={ss.softskill_id} value={ss.softskill_id}>
                      {ss.softskill_name}
                    </option>
                  ))}
                </select>
