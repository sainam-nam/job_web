...
const WorkExEdit = () => {
      ...
      useEffect(() => {
          const fetchEduData = async () => {
            if (!userId) return;
            try {
              const res = await axios.get(`${apiUrl}/user_workexdata?jobber_id=${userId}&no=${no}`);
              if (res.data && res.data.data && res.data.data.length > 0) {
                const workex = res.data.data[0]; // ดึงรายการแรก
                setFormData({
                  position_id: workex.position_id,
                  company: workex.company || "",
                  start: workex.start || "",
                  end: workex.end || "",
                  job_description: workex.job_description || "",
                  file: workex.work_certificate || ""  // ไฟล์จะอัปโหลดใหม่เท่านั้น ไม่โหลดเก่า
                });
                //console.log(edu);
                setOldFile(workex.work_certificate || "");
                setSelectedJobType(workex.jobtype_id);
              }
            } catch (error) {
      ...
      useEffect(() => {
        ...
        const handleSubmit = async (e) => {
            ...
            try {
                const res = await axios.post(`${apiUrl}/api/user_workex_update`, form, {
                headers: {
                  ...
                  <form onSubmit={handleSubmit} className='mt-2 '>
                    <div className="flex flex-col gap-2 items-center justify-center ">
                        ...
                        {/* ตำแหน่งงาน */}
                            ...
                            <input type="hidden" name="old_file" value={oldFile} />        
                            <div className='my-2'>
                            <a className="text-[#8E80FF] font-bold mt-2">หลักฐานรับรองการทำงาน   </a>
                                      {oldFile && (
                                        
                                            <a
                                              href={`/uploads/user_workfile/${oldFile}`}
                                              target="_blank"
