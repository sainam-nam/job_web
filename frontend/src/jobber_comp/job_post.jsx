...
async function checkIfApplied() {
  try {
    const res = await fetch(`${apiUrl}/api/check-apply?jobber_id=${userId}&post_id=${id}&type=j`);
...
async function handleApplyToggle(post_id) {
  try {
    if (applied) {
      // ลบออก
      await fetch(`${apiUrl}/api/apply-job`, {
        method: "DELETE",
      ...
      const res = await fetch(`${apiUrl}/api/apply-job`, {
        method: "POST",
        headers: {
...
async function checkstatus() {
  try {
    const res = await fetch(`${apiUrl}/api/check-apply?jobber_id=${userId}&post_id=${id}`);
    const data = await res.json();
...
async function checkIfFav() {
  try {
    const res = await fetch(`${apiUrl}/api/check-apply?jobber_id=${userId}&post_id=${id}&type=f`);
    const data = await res.json();
  ...
  function formatAgeRange(ageStr) {
  if (!ageStr) return "";

  const [min, max] = ageStr.split("-").map(Number);

  if (min === 0 && max === 0) return "ไม่กำหนดอายุ";
  if (min !== 0 && max === 0) return `${min} ปีขึ้นไป`;
  if (min === 0 && max !== 0) return `ไม่เกิน ${max} ปี`;
  return `${min}-${max} ปี`;
}
    ...
    return (
      <div>
        ...
        {alertMessage && (
          <div
            role="alert"
            className={`alert alert-${alertType} mt-2`}
          >
            <svg
            ...
            <div className="flex flex-col gap-1 lg:gap-2 xl:gap-3">
              ...
              <div className="flex justify-end gap-2 mt-2 md:mt-5">
                ...
                <button 
                  ...
                  className="btn btn-xs md:btn-sm lg:btn-lg xl:btn-xl w-20 md:w-25 lg:w-35 xl:w-45 border border-white bg-white text-[#8E80FF] rounded-xl md:rounded-2xl lg:rounded-3xl xl:rounded-4xl"
                >ดูโปร์ไฟล์</button>
              </div>
        ...
        <div className="bg-white px-5 py-3 md:px-15 md:py-6 lg:px-25 lg:py-12 xl:px-35 xl:py-14">
          <div className="flex flex-col justify-center w-full mb-2 p-4 md:mb-4 md:p-8 lg:mb-8 lg:p-10 xl:pt-14 bg-[#D9D9D9] text-[#8E80FF] rounded-3xl lg:rounded-4xl" style={{ boxShadow: '0 0 10px rgba(0,0,0,0.2)' }}>
          ...
          <div className="py-2 md:py-4 lg:py-6">
            ...
            <div className="text-[10px] md:text-sm lg:text-xl xl:text-2xl text-[#8E80FF] ml-4 md:ml-10">
              <b>อายุ</b> {formatAgeRange(data[0]?.age)}
                  ...
                  {status === "waitemp" ? (
                   <span className="text-[#8E80FF] text-4xl font-bold">รอการตอบกลับจากนายจ้าง</span>
                    ...
                    <button
                      ...
                      className={`btn btn-xs md:btn-sm lg:btn-lg border-green-500 w-30 p-2 md:p-3 lg:p-4 xl:p-5 ${
                        applied ? "bg-red-500" : "bg-green-500"
                      } text-white rounded-xl md:rounded-2xl lg:rounded-3xl xl:rounded-4xl`}
                    >
                      {applied ? "ยกเลิก" : "สมัครงาน"}
                    </button>
                    <a className="text-red-500">
                      {applied
                        ? "ท่านสามารถยกเลิกการสมัครได้ หากเปลี่ยนความประสงค์"
                        : "โปรดตรวจสอบรายละเอียดงานให้ครบถ้วนก่อนกดสมัคร"}
                    </a>
                  </>
                  ) : status === "accepted" ? (
                    <>
