...
const InterVolunEdit = () => {
      const { inter_volun_id } = useParams();
      ...
      useEffect(() => {
          const fetchEduData = async () => {
            if (!userId) return;
            try {
              const res = await axios.get(`${apiUrl}/user_intervolundata?inter_volun_id=${inter_volun_id}`);
              if (res.data && res.data.data && res.data.data.length > 0) {
                const intervolun = res.data.data[0]; // ดึงรายการแรก
                setFormData({
                  voluntype_id: intervolun.voluntype_id,
                  start: intervolun.hours || "",
                  end: intervolun.end_hour || "",
                  ready_travel: intervolun.ready_travel || "",
                  health_limit: intervolun.health_limit || "",
                  experience: intervolun.experience || ""
                });
                if (intervolun.days) {
                  const converted = convertDaysToWeekdays(intervolun.days);
      ...
      useEffect(() => {
        ...
        const handleSubmit = async (e) => {
            ...
            form.append("inter_volun_id", inter_volun_id); 
            ...
            try {
                const res = await axios.post(`${apiUrl}/api/user_intervolun_update`, form, {
                headers: {
      ...
      <div>
         <div className='text-[#8E80FF] bg-white rounded-3xl p-6 relative' style={{ boxShadow: '0 0 10px rgba(0,0,0,0.2)' }}>
              <div className="flex gap-4 items-center mb-4">
                  <h2 className="font-bold text-xl">แก้ไขข้อมูลการสมัครกิจกรรมจิตอาสา</h2>
              </div>
            ...
            <div className="modal-box bg-white">
                <center>
                    ...
                    <p className=" text-[#8E80FF]">คุณได้แก้ไขข้อมูลการสมัครกิจกรรมจิตอาสาเรียบร้อย </p>
