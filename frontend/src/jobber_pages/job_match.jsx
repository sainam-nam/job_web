...
function Job_Match() {
  ...
  const inter_work_id = queryParams.get("inter_work");
  ...
  useEffect(() => {
          MatchList_fetchData();
        }, [userData , id , inter_work_id]); //ต่อนี่ ดึงข้อมูลมาโชว์ แล้วให้แยก 0 กับ 1 แล้วเอางานมาโชว์ด้วย
  ...
  const MatchList_fetchData = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/job_matchlist?jobber_id=${userId}&post_id=${id}&inter_work_id=${inter_work_id}&position_id=${position_id}`);
      const result = await res.json();
        ...
        <div>
          ...
          {showDe && (
            ...
            <div className="py-2 md:py-4 lg:py-6">
              ...
              <div className="text-[10px] md:text-sm lg:text-xl xl:text-2xl text-[#8E80FF] ml-4 md:ml-10">อายุ {data[0]?.age}</div>
              <div className="text-[10px] md:text-sm lg:text-xl xl:text-2xl text-[#8E80FF] ml-4 md:ml-10">
                เพศ
                {data[0]?.gender === "M"
                        ? " ชาย"
