...
export default function VolunsByvoluntype() {
  const { voluntype } = useParams();
  ...
  }, [limit, page, search, voluntype]);

  const fetchData = async () => {
    try {
      const res = await fetch(`${apiUrl}/api/voluns?voluntype=${voluntype}&page=${page}&limit=${limit}&keyword=${search}`);
  ...
  return (
    <div className="flex flex-col justify-center items-center w-full px-5 pt-3 lg:pt-5 lg:px-0 lg:pr-4">
          <h1 className="text-lg md:text-3xl font-bold mb-2 text-[#7B6ADA] ">{voluntype} [ {total} ]</h1>
