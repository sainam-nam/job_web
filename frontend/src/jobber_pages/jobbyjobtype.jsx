...
export default function JobsByjobtype() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`http://localhost:8081/api/jobs?jobtype=${jobtype}&page=${page}&limit=${limit}&keyword=${search}`);
  ...
  return (
    <div className="flex flex-col justify-center items-center w-full p-5">
          <h1 className="text-3xl font-bold mb-4 text-[#7B6ADA] ">{jobtype} [ {total} ]</h1>
