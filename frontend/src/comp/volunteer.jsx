...
export default function Volunteer() {
  ...
  const [editData, setEditData] = useState({ post_id: "", activity_name: "" , employer: "" , status: "" });
    ...
    <main className="flex-1 pt-2 px-2 lg:p-8">
        ...
        <table className="table w-full">
          <thead>
            <tr className="bg-[#D9D9D9] text-[10px] md:text-lg text-[#7B6ADA] font-bold text-center">
              ...
              <th className="w-[20%]">ชื่อกิจกรรม</th>
              <th className="w-[15%]">ผู้จัด</th>
          ...
          <tbody>
            {data.map((val, index) => (
              <tr key={index} className="hover:bg-[#D9D9D9] text-[10px] md:text-lg">
                ...
                <td className="">{val.activity_name || "N/A"}</td>

                
                <td className="text-center">{val.employer}</td>    
                <td className="text-center hidden md:table-cell">{val.voluntype_name}</td>
      ...
      <dialog id="status_modal" className="modal">
        <div className="modal-box bg-white">
          <center>
              {editData.status === "ON" ? (
                      <a className="text-xs md:text-lg text-error font-bold mb-4">ต้องการระงับการใช้งานของกิจกรรมจิตอาสานี้ใช่หรือไม่?</a> 
                  ) : (
                     <a className="text-xs md:text-lg text-success font-bold mb-4">ต้องการคืนสิทธิ์การใช้งานของกิจกรรมจิตอาสานี้ใช่หรือไม่?</a>  
                  )}
              
          </center>   
          
            <div className="flex items-center">
              <label className="text-xs md:text-md text-[#7B6ADA]">ชื่อกิจกรรม : </label>
              &nbsp;&nbsp;
              <h3 className="text-xs text-[#7B6ADA] font-bold">{editData.activity_name}</h3>
            </div>
            <div className="flex items-center">
              <label className="text-xs md:text-md text-[#7B6ADA]">ผู้จัด : </label>
              &nbsp;&nbsp;
