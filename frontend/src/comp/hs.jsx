...
export default function HS() {
  ...
  const [editData, setEditData] = useState({ hardskill_id: "", hardskill_name: "" });
  const [addData, setAddData] = useState({ hardskill_id: "", hardskill_name: "" });
  const [delData, setDelData] = useState({ hardskill_id: "", hardskill_name: "" });
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/hardskill?page=${page}&limit=${limit}&keyword=${search}`);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/hardskill/${editData.hardskill_id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ hardskill_name: editData.hardskill_name }),
      });
  ...
  const handleInsert = async () => {
    if (!addData.hardskill_name.trim()) {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/hardskill/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ hardskill_name: addData.hardskill_name }),
      });
  ...
  const handleDelete = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/hardskill/${delData.hardskill_id}`, {
        method: "DELETE",
  ...
  return (
    <main className="flex-1 pt-2 px-4 lg:p-8">
      ...
      <div className="flex justify-between items-center mt-1 mb-1 md:mt-2 md:mb-2">
        ...
        <button className="btn btn-xs sm:btn-sm md:btn-sm btn-success text-[10px] text-white px-4 py-2 rounded-lg" onClick={() => {setAddData({ hardskill_id: "", hardskill_name: "" }); document.getElementById("add_modal").showModal();}} >เพิ่มข้อมูล</button>
        <div className="flex items-center gap-2">
        <label className="input w-30 h-7 bg-white rounded-lg border border-[#7B6ADA] ">
          
          <div className="tooltip tooltip-bottom" data-tip="ค้นหาจากทักษะความรู้">
            <input 
              type="search" 
      ...
      <center>
        <div className="join items-center gap-2 my-2">
          {page > 1 && (
            <button onClick={() => setPage(page - 1)}>
              <img src="/up.png" className="-rotate-90 w-4 h-4 md:w-6 md:h-6 rounded hover:shadow-lg hover:shadow-[#7B6ADA] transition-shadow" />
        ...
        {total > 0 ? (
          ...
          {page < totalPages && (
            <button onClick={() => setPage(page + 1)}><img src="/down.png" className="-rotate-90 w-4 h-4 md:w-6 md:h-6 rounded hover:shadow-lg hover:shadow-[#7B6ADA] transition-shadow" /></button>
          )}
        </div>
      </center>
      {/* Modal สำหรับแก้ไข */}
      <dialog id="my_modal" className="modal">
        <div className="modal-box bg-white">
          <center><h2 className="text-xs md:text-lg text-[#7B6ADA] font-bold mb-4">แก้ไขทักษะด้านความรู้</h2></center>
          <input
            type="text"
            className="w-full text-sm md:text-md text-[#7B6ADA] border border-gray-300 rounded-md px-3 py-2 mt-2"
            value={editData.hardskill_name}
            onChange={(e) => setEditData({ ...editData, hardskill_name: e.target.value })}
          />
      ...
      <dialog id="add_modal" className="modal">
        <div className="modal-box  bg-white">
          <center><h2 className="text-xs md:text-lg text-[#7B6ADA] font-bold mb-2 md:mb-4">เพิ่มทักษะด้านความรู้</h2></center>
          <input
            type="text"
            className="w-full text-sm md:text-md text-[#7B6ADA] border border-gray-300 rounded-md px-3 py-2 mt-2"
            value={addData.hardskill_name}
            onChange={(e) => setAddData({ ...addData, hardskill_name: e.target.value })}
          />
          <div className="modal-action">
      ...
      <dialog id="del_modal" className="modal">
        <div className="modal-box bg-white">
          <center><h2 className="text-xs md:text-lg text-[#7B6ADA] font-bold mb-4">คุณต้องการลบข้อมูลทักษะด้านความรู้นี้</h2></center>
            <div className="flex flex-col">
              <label className="text-xs md:text-md text-[#7B6ADA]">ชื่อข้อมูลทักษะด้านความรู้ : </label>
              <h3 className="text-xs text-[#7B6ADA] font-bold">{delData.hardskill_name}</h3>
            </div>
          <div className="modal-action">
