...
export default function VolunType() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`http://localhost:8081/voluntype?page=${page}&limit=${limit}&keyword=${search}`);
  ...
  const handleUpdate = async () => {
    try {
      const res = await fetch(`http://localhost:8081/voluntype/${editData.voluntype_id}`, {
        method: "PUT",
        headers: {
  ...
  const handleStatus = async () => {
    try {
      const res = await fetch(`http://localhost:8081/voluntypesta/${editData.voluntype_id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleInsert = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8081/voluntype/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`http://localhost:8081/voluntype/${delData.voluntype_id}`, {
        method: "DELETE",
  ...
  return (
    <main className="flex-1 pt-2 px-4 lg:p-8">
      ...
      <div className="flex justify-between items-center mt-1 mb-3 md:mt-2 md:mb-2">
        ...
        <button className="btn btn-xs sm:btn-lg md:btn-sm btn-success text-[10px] text-white px-4 py-2 rounded-lg" onClick={() => {setAddData({ voluntype_id: "", voluntype_name: "" }); document.getElementById("add_modal").showModal();}} >เพิ่มข้อมูล</button>
