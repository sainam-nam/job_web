...
export default function Job() {
  ...
  const [editData, setEditData] = useState({ post_id: "", position_name: "" , employer: "" , status: "" });
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/job?page=${page}&limit=${limit}&keyword=${search}`);
  ...
  const handleStatus = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/jobsta/${editData.post_id}`, {
        method: "PUT",
        headers: {
    ...
    <main className="flex-1 pt-2 px-2 lg:p-8">
      ...
      <div className="flex justify-between items-center mt-1 mb-3 md:mt-2 md:mb-2">
        ...
        <label className="input w-30 h-7 bg-white rounded-lg border border-[#7B6ADA]">
          ...
          <div className="tooltip tooltip-bottom" data-tip="ค้นหาจากตำแหน่ง">
            <input 
              type="search" 
        ...
        <table className="table w-full">
          <thead>
            <tr className="bg-[#D9D9D9] text-[10px] md:text-lg text-[#7B6ADA] font-bold text-center">
              ...
              <th className="w-[15%]">นายจ้าง</th>
              <th className="w-[20%] hidden md:table-cell">ประเภท</th>
              <th className="w-[5%]">การจับคู่</th>
              <th className="w-[5%] hidden md:table-cell">การใช้งาน</th>
          ...
          <tbody>
            {data.map((val, index) => (
              <tr key={index} className="hover:bg-[#D9D9D9] text-[10px] md:text-lg">
                ...
                <td className="text-center">{val.employer}</td>    
                <td className="text-center hidden md:table-cell">{val.jobtype_name}</td>
      ...
      <dialog id="status_modal" className="modal">
        <div className="modal-box bg-white">
          <center>
              {editData.status === "ON" ? (
                      <a className="text-xs md:text-lg text-error font-bold mb-4">ต้องการระงับการใช้งานของงานนี้ใช่หรือไม่?</a> 
                  ) : (
                     <a className="text-xs md:text-lg text-success font-bold mb-4">ต้องการคืนสิทธิ์การใช้งานของงานนี้ใช่หรือไม่?</a>  
                  )}
              
          </center>   
          
            <div className="flex items-center">
              <label className="text-xs md:text-md text-[#7B6ADA]">ตำแหน่ง : </label>
              &nbsp;&nbsp;
              <h3 className="text-xs text-[#7B6ADA] font-bold">{editData.position_name}</h3>
            </div>
            <div className="flex items-center">
              <label className="text-xs md:text-md text-[#7B6ADA]">นายจ้าง : </label>
              &nbsp;&nbsp;
              <h3 className="text-xs text-[#7B6ADA] font-bold">{editData.employer}</h3>
            </div>
            <div className="flex items-center">
