...
export default function JobType() {
  ...
  const fetchData = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/jobtype?page=${page}&limit=${limit}&keyword=${search}`);
  ...
  const handleUpdate = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/jobtype/${editData.jobtype_id}`, {
        method: "PUT",
        headers: {
  ...
  const handleStatus = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/jobtypesta/${editData.jobtype_id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleInsert = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/jobtype/`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleDelete = async () => {
    try {
      ...
      const res = await fetch(`${apiUrl}/jobtype/${delData.jobtype_id}`, {
        method: "DELETE",
  ...
  return (
    <main className="flex-1 pt-2 px-4 lg:p-8">
      ...
      <div className="flex justify-between items-center mt-1 mb-3 md:mt-2 md:mb-2">
        ...
        <button className="btn btn-xs sm:btn-sm md:btn-sm btn-success text-[10px] text-white px-4 py-2 rounded-lg" onClick={() => {setAddData({ jobtype_id: "", jobtype_name: "" }); document.getElementById("add_modal").showModal();}} >เพิ่มข้อมูล</button>
        ...
        <label className="input w-30 h-7 bg-white rounded-lg border border-[#7B6ADA]">
          <div className="tooltip tooltip-bottom" data-tip="ค้นหาจากประเภทงาน">
          <input 
            type="search" 
