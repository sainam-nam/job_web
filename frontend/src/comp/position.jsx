...
export default function Position() {
  ...
  const [editData, setEditData] = useState({ position_id: "", position_name: "", jobtype_id: "", status: "" });
  const [addData, setAddData] = useState({ position_id: "", position_name: "", jobtype_id: "" });
  const [delData, setDelData] = useState({ position_id: "", position_name: "" });
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/position/${editData.position_id}`, {
        ...
        body: JSON.stringify({ 
          ...
          jobtype_id: editData.jobtype_id
        }),
      });
  ...
  const handleInsert = async () => {
    if (!addData.position_name.trim()) {
      alert("กรุณากรอกชื่อตำแหน่งงาน");
      return;
    }
    if(addData.jobtype_id == "") {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/position/`, {
        ...
        body: JSON.stringify({ 
          position_name: addData.position_name, 
          jobtype_id: addData.jobtype_id
        }),
      });
  ...
  return (
    <main className="flex-1 pt-2 px-4 lg:p-8">
      ...
      <div className="flex justify-between items-center mt-1 mb-3 md:mt-2 md:mb-2">
        ...
        <button className="btn btn-xs sm:btn-lg md:btn-sm btn-success text-[10px] text-white px-4 py-2 rounded-lg" onClick={() => {setAddData({ position_id: "", position_name: "" }); document.getElementById("add_modal").showModal();}} >เพิ่มข้อมูล</button>
        ...
        <table className="table">
          <thead>
            <tr className="bg-[#D9D9D9] text-[10px] md:text-lg text-[#7B6ADA] font-bold text-center">
              ...
              <th className="w-[35%]">ตำแหน่งงาน</th>
              <th className="w-[35%]">ประเภท</th>
              <th className="w-[10%]">การใช้งาน</th>
              <th className="w-[15%]">จัดการ</th>
            </tr>
          </thead>
          <tbody>
            {data.map((val, index) => (
              <tr key={index} className="hover:bg-[#D9D9D9] text-[10px] md:text-[15px]">
                ...
                <td className="">{val.position_name || "N/A"}</td>
                <td className="text-center">{val.jobtype_name || "N/A"}</td>
                ...
                <td className=" flex flex-col gap-0.5 items-center md:gap-1 md:text-center lg:block lg:flex lg:flex-row lg:gap-1 lg:items-center">
      ...
      <dialog id="my_modal" className="modal">
        <div className="modal-box bg-white">
          <center><h2 className="text-xs md:text-lg text-[#7B6ADA] font-bold mb-4">แก้ไขตำแหน่งงาน</h2></center>
          <input
            type="text"
            ...
            onChange={(e) => setEditData({ ...editData, position_name: e.target.value })}
          />
          <a className="text-sm md:text-md text-[#7B6ADA]">ประเภทงาน</a>
          <select 
            className="select w-full bg-white text-[#7B6ADA] border border-gray-300 rounded-md px-3 py-2 mt-2"
            value={editData.jobtype_id}
            onChange={(e) => setEditData({...editData, jobtype_id: e.target.value })}
          >
            
            {jobtypedata.map((job) => (
              <option key={job.jobtype_id} value={job.jobtype_id}>
                {job.jobtype_name}
              </option>
            ))}
          </select>
      ...
      <dialog id="add_modal" className="modal">
        <div className="modal-box bg-white">
          <center><h2 className="text-xs md:text-lg text-[#7B6ADA] font-bold mb-2 md:mb-4">เพิ่มตำแหน่งงาน</h2></center>
          <a className="text-sm md:text-md text-[#7B6ADA]">ชื่อตำแหน่งงาน</a>
          <input
            type="text"
            className="w-full text-sm md:text-md text-[#7B6ADA] border border-gray-300 rounded-md px-3 py-2 mt-2"
            value={addData.position_name}
            onChange={(e) => setAddData({ ...addData, position_name: e.target.value })}
          />
          <a className="text-sm md:text-md text-[#7B6ADA]">ประเภทงาน</a>
          <select 
            className="select w-full bg-white text-[#7B6ADA] border border-gray-300 rounded-md px-3 py-2 mt-2"
            value={addData.jobtype_id}
            onChange={(e) => setAddData({...addData, jobtype_id: e.target.value })}
          >
      ...
      <dialog id="del_modal" className="modal">
        <div className="modal-box  bg-white">
          <center><h2 className="text-xs md:text-lg text-[#7B6ADA] font-bold mb-4">คุณต้องการลบข้อมูลตำแหน่งงานนี้</h2></center>
            <div className="flex">
              <label className="text-xs md:text-md text-[#7B6ADA]">ชื่อตำแหน่งงาน : </label>
              &nbsp;&nbsp;
              <h3 className="text-xs text-[#7B6ADA] font-bold">{delData.position_name}</h3>
            </div>
          <div className="modal-action">
      ...
      <dialog id="status_modal" className="modal">
        <div className="modal-box bg-white">
          <center>
              {editData.status === "ON" ? (
                      <a className="text-xs md:text-lg text-error font-bold mb-4">ต้องการระงับตำแหน่งงานนี้ใช่หรือไม่?</a> 
                  ) : (
                     <a className="text-xs md:text-lg text-success font-bold mb-4">ต้องการอนุมัติตำแหน่งงานนี้ใช่หรือไม่?</a>  
                  )}
            ...
            <div className="flex items-center">
              <label className="text-xs md:text-md text-[#7B6ADA]">ชื่อตำแหน่งงาน : </label>
              &nbsp;&nbsp;
              <h3 className="text-xs text-[#7B6ADA] font-bold">{editData.position_name}</h3>
            </div>
            <div className="flex items-center">
